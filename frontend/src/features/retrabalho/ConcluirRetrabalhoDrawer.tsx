      ...
      <form onSubmit={handleSubmit} className="space-y-5 text-sm">
        ...
        <div className="p-3.5 rounded-lg bg-surface-elevated border border-surface-border space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500 dark:text-gray-400">Equipamento em Reparo Corretivo</span>
          ...
          <div className="flex justify-between text-xs text-gray-600 dark:text-gray-300 pt-1 border-t border-surface-border/50">
        ...
        <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-300 dark:border-amber-500/30 space-y-2.5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
              <ShieldAlert className="w-4 h-4" /> Laudo do CQ / Motivo de Reprovação
            </span>
            {retrabalho.motivoReprovacao && (
              <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-700 dark:text-amber-300 text-[10px] font-bold border border-amber-500/40">
                {retrabalho.motivoReprovacao.codigo}
              </span>
            )}
          </div>

          <p className="text-xs text-gray-900 dark:text-gray-200 font-semibold">
            {retrabalho.motivoReprovacao?.descricao || 'Defeito identificado no ensaio funcional'}
          </p>

          {retrabalho.detalhesDefeito && (
            <div className="p-2 rounded bg-surface-base/80 border border-surface-border/60 text-xs text-gray-700 dark:text-gray-300">
              <span className="text-gray-500 dark:text-gray-400 font-semibold">Instruções do Inspetor:</span>{' '}
              {retrabalho.detalhesDefeito}
            </div>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold uppercase text-gray-700 dark:text-gray-300 tracking-wider">
            Solução Técnica Aplicada no Retrabalho <span className="text-red-500 dark:text-red-400">*</span>
          </label>
          <textarea
            ...
            required
            className="w-full bg-surface-base border border-surface-border rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 resize-none"
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            className="w-full bg-surface-base border border-surface-border rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 resize-none"
          />
        </div>

        <div className="p-3 rounded-lg bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-500/20 text-xs text-purple-700 dark:text-purple-300">
          <p className="font-semibold flex items-center gap-1.5">
            <RotateCcw className="w-3.5 h-3.5 text-purple-500 dark:text-purple-400" /> Fluxo Operacional de Re-teste:
          </p>
          <p className="mt-1 text-gray-600 dark:text-gray-400">
            Ao concluir, o lote assumirá o status <strong className="text-purple-700 dark:text-purple-300">Aguardando Reteste</strong> e retornará com prioridade para a bancada do Controle de Qualidade (CQ).
          </p>
        </div>
      </form>
    </Drawer>
