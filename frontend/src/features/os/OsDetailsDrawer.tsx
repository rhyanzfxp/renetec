      ...
      <div className="space-y-6 text-sm">
        ...
        <div className="p-4 rounded-xl bg-surface-elevated border border-surface-border space-y-3">
          ...
          <div className="flex items-center justify-between border-t border-surface-border/50 pt-2 text-xs">
            <span className="text-gray-500 dark:text-gray-400">Prioridade da OS:</span>
            <StatusBadge prioridade={os.prioridade} size="sm" />
          </div>
          {os.valorOrcamento && (
            <div className="flex items-center justify-between border-t border-surface-border/50 pt-2 text-xs">
              <span className="text-gray-500 dark:text-gray-400">Valor Orçado:</span>
          ...
          <div className="space-y-2.5">
            {os.itens.map((item, idx) => (
              <div key={item.id || idx} className="p-3.5 rounded-lg bg-surface-card border border-surface-border space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-gray-900 dark:text-white text-sm">{item.tipoEquipamento.nome}</span>
                  <span className="px-2 py-0.5 rounded bg-brand-500/10 text-brand-700 dark:text-brand-400 text-xs font-bold tabular-nums border border-brand-500/30">
                    {item.quantidade} un
                  </span>
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  <span className="text-gray-700 dark:text-gray-300 font-medium">Técnico Alocado: </span>
                  {item.tecnicoAlocado?.nome || 'Não atribuído'}
                </div>
                <div className="p-2.5 rounded bg-surface-base border border-surface-border/80 text-xs text-gray-700 dark:text-gray-300">
                  <span className="font-semibold text-gray-500 dark:text-gray-400 block mb-1">Defeito Relatado:</span>
        ...
        {os.observacoes && (
          <div className="space-y-1">
            ...
            <p className="p-3 rounded-lg bg-surface-card border border-surface-border text-xs text-gray-700 dark:text-gray-300">
              {os.observacoes}
            </p>
          </div>
        )}
        ...
        {/* Timestamps */}
        <div className="space-y-2 border-t border-surface-border pt-4 text-xs text-gray-500 dark:text-gray-400">
          <div className="flex justify-between">
            <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> Entrada no Sistema:</span>
            <span className="text-gray-800 dark:text-gray-200 tabular-nums">{new Date(os.dataEntrada).toLocaleString('pt-BR')}</span>
          </div>
        </div>
      </div>
    </Drawer>
